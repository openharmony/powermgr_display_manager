    ...
    int displayId, std::unordered_map<std::string, LuxFilterConfig::Data>& data)
{
    ...
    for (auto value : root) {
        ...
        if (!value["filterName"].isString() || value["filterName"].asString().empty()) {
            DISPLAY_HILOGW(FEAT_BRIGHTNESS, "<%{public}s> filterName is not find!", CONFIG_NAME.c_str());
            continue;
        }
        std::string name = value["filterName"].asString();
        if (value["filterNoFilterNum"].isInt()) {
            config.filterNoFilterNum = value["filterNoFilterNum"].asInt();
        }
        if (value["filterNum"].isInt()) {
            config.filterNum = value["filterNum"].asInt();
        }
        if (value["filterMaxFuncLuxNum"].isInt()) {
            config.filterMaxFuncLuxNum = value["filterMaxFuncLuxNum"].asInt();
        }
        if (value["filterAlpha"].isNumeric()) {
            config.filterAlpha = value["filterAlpha"].asFloat();
        }
        if (value["filterLuxTh"].isInt()) {
            config.filterLuxTh = value["filterLuxTh"].asInt();
        }
